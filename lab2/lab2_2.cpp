...
double df1_x1(double x1) {
    return 4 * x1;
}

double df1_x2(double x2) {
    return -1 + 2 * x2;
}
...
pair<Matrix, int> simple_iterations_system(double ax1, double bx1, double ax2, double bx2, Matrix X, double eps) {
    int n = X.GetRows();
    Matrix X_Prev(n, 1), J(n, n), T(n, n), E(n, n);
    J(0, 0) = df1_x1(X(0, 0));
    J(0, 1) = df1_x2(X(1, 0));
    J(1, 0) = df2_x1(X(0, 0));
    J(1, 1) = df2_x2(X(1, 0));
    T = J.InverseMatrix(); // T = J^-1(X0)
    for (int i = 0; i < n; ++i) {
        E(i, i) = 1;
    }
    Matrix M(n, n);
    M(0, 0) = df1_x1(golden_ratio(ax1, bx1, eps, df1_x1));
    M(0, 1) = df1_x2(golden_ratio(ax2, bx2, eps, df1_x2));
    M(1, 0) = df2_x1(golden_ratio(ax1, bx1, eps, df2_x1));
    M(1, 1) = df2_x2(golden_ratio(ax2, bx2, eps, df2_x2));
    E.SubMatrix(T.MulMatrixReturn(M)); // J_phi = E - T * M
    double q = E.norm();
    int k = 0;
    Matrix F(n, 1);
    do {
        X_Prev = X;
        F(0, 0) = f1(X_Prev(0, 0), X_Prev(1, 0));
        F(1, 0) = f2(X_Prev(0, 0), X_Prev(1, 0));
        // X = X - T * F
        X.SubMatrix(T.MulMatrixReturn(F));
        ++k;
