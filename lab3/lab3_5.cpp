...
int main() {
    ...
    double F = runge_romberg_method(F1, F2, h1, h2, 10);
    cout << "F = " << F1 << " with h = " << h1 << " error = " << abs(F - F1) << "\n";
    cout << "F = " << F2 << " with h = " << h2 << " error = " << abs(F - F2) << "\n";
    cout << "\nTrapeze method:\n";
    F1 = trapeze_method(a, b, h1);
    F2 = trapeze_method(a, b, h2);
    F = runge_romberg_method(F1, F2, h1, h2, 10);
    cout << "F = " << F1 << " with h = " << h1 << " error = " << abs(F - F1) << "\n";
    cout << "F = " << F2 << " with h = " << h2 << " error = " << abs(F - F2) << "\n ";
    cout << "\nSimpson method:\n";
    ...
    F = runge_romberg_method(F1, F2, h1, h2, 10);
    cout << "F = " << simpson_method(a, b, h1) << " with h = " << h1 << " error = " << abs(F - F1) << "\n";
    cout << "F = " << simpson_method(a, b, h2) << " with h = " << h2 << " error = " << abs(F - F2) << "\n ";
